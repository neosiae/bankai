...
function node (state, createEdge) {
  ...
  serverRender(entry, function (err, render) {
    ...
    function getTemplate (language, done) {
      var dir = path.join(path.dirname(entry))
      resolve('.', { basedir: dir, extensions: ['.html'] }, function (err, filename) {
        if (err) {
          done(head(language), dir)
        } else {
    ...
    function renderRoute (route, done) {
      ...
      getTemplate(language, ontemplate)

      function ontemplate (html, filename) {
